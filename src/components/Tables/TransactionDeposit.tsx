...
interface TransactionDepositProps {
  data: Transaction[];
}

const TransactionDeposit: React.FC<TransactionDepositProps> = ({ data }) => {
  ...
  const filteredData = data.filter((transaction) => {
    const { code, walletAddress, displayName, hash } = transaction;
    return (
      code.toLowerCase().includes(searchQuery) ||
      walletAddress.toLowerCase().includes(searchQuery) ||
      (displayName && displayName.toLowerCase().includes(searchQuery)) ||  // Handle null displayName
      (hash && hash.toLowerCase().includes(searchQuery))  // Handle null hash
    );
  });
            ...
            {currentPageData.map((packageItem, key) => (
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] py-5 px-4  dark:border-strokedark ">
                  ...
                  <p className="text-sm">{formatWalletAddress(packageItem.displayName)}</p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">
                    {packageItem.network}
